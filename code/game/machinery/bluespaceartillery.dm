...
/obj/machinery/artillerycontrol
	var/reload = 180
	name = "onispace artillery control"
...
/obj/machinery/artillerycontrol/attack_hand(mob/user as mob)
	...
	var/dat = "<B>onispace Artillery Control:</B><BR>"
	dat += "Locked on<BR>"
...
/obj/machinery/artillerycontrol/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (issilicon(usr)))
		...
		if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (issilicon(usr)))
			command_announcement.Announce("onispace artillery fire detected. Brace for impact.")
...
/*mob/proc/openfire()
	...
	A = input("Area to jump bombard", "Open Fire", A) in SSmapping.teleportlocs
	var/area/thearea = SSmapping.teleportlocs[A]
	command_alert("onispace artillery fire detected. Brace for impact.")
	spawn(30)
	var/list/L = list()
